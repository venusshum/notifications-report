...
DOCKER_BUILDER_IMAGE_NAME = govuk/notify-reports-builder:${DOCKER_IMAGE_TAG}
DOCKER_TTY ?= $(if ${JENKINS_HOME},,t)

BUILD_TAG ?= notifications-reports-manual
BUILD_NUMBER ?= 0
...
CF_REPORTS ?= reports.cloud.service.gov.uk
CF_ORG ?= govuk-notify
CF_SPACE ?= ${DEPLOY_ENV}
CF_HOME ?= ${HOME}
$(eval export CF_HOME)

CF_MANIFEST_FILE = manifest-$(subst notify-,,${CF_APP})-${CF_SPACE}.yml
...
cf-deploy: ## Deploys the app to Cloud Foundry
	$(if ${CF_SPACE},,$(error Must specify CF_SPACE))
	$(if ${CF_APP},,$(error Must specify CF_APP))
	echo ${CF_MANIFEST_FILE}
	@cf app --guid ${CF_APP} || exit 1
	cf rename ${CF_APP} ${CF_APP}-rollback
	make -s generate-manifest >> manifest_file.yml
	cf push ${CF_APP} -f manifest_file.yml
	rm manifest_file.yml
...
.PHONY: cf-deploy-reports-db-migration
cf-deploy-reports-db-migration:
	$(if ${CF_SPACE},,$(error Must specify CF_SPACE))
	cf unbind-service notify-reports-db-migration notify-reports
	cf push notify-reports-db-migration -f <(make -s CF_APP=notify-reports-api generate-manifest)
	cf run-task notify-reports-db-migration "flask db upgrade" --name reports_db_migration
